    ...
    <div className="fixed bottom-5 right-5 z-50">
      {open && (
        <motion.div
          ...
          className="mb-3 w-[360px] overflow-hidden rounded-2xl border bg-card shadow-2xl"
        >
          <div className="flex items-center gap-2 bg-gradient-to-r from-primary to-secondary px-4 py-3 text-primary-foreground">
            <Sparkles className="h-4 w-4" />
            <div className="text-sm font-semibold">AI Assistant</div>
          </div>
          <div className="p-3">
          <div className="mb-3 h-64 space-y-2 overflow-y-auto rounded-xl bg-muted/50 p-3">
            {messages.length === 0 && <div className="text-xs text-muted-foreground">Ask: What should I do next?</div>}
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`rounded-2xl px-3 py-2 text-sm ${msg.role === "user" ? "ml-8 bg-primary text-primary-foreground" : "mr-8 border bg-card text-foreground"}`}
              >
                {msg.content}
              </div>
            ))}
      ...
      <Button onClick={() => setOpen((v) => !v)} className="h-12 w-12 rounded-full p-0 shadow-soft">
